...
def service_incident_title_zh(title: str) -> str:
    ...
    exact = {
        "Elevated Error Rates": "OpenAI 服务错误率升高",
...
def fetch_anthropic_public(session: requests.Session, now: datetime) -> list[RawItem]:
    """Read only Anthropic's public news page, with visible parser failures."""
    response = session.get("https://www.anthropic.com/news", timeout=20)
    response.raise_for_status()
    items = parse_anthropic_news_items(response.text, now)
    if not items:
        raise ValueError("No dated Anthropic news items parsed")
    return items
...
def collect_all(session: requests.Session, now: datetime, *, public_only: bool = False) -> tuple[list[RawItem], list[dict[str, Any]]]:
    ...
    for site_id, site_name, fn in tasks:
        ...
        try:
            ...
            # Public status must not echo request headers, URLs with credentials,
            # or arbitrary server error bodies.
            error = type(exc).__name__ if public_only else str(exc)
...
def fetch_opml_rss(
    ...
    *,
    ...
    def fetch_single_feed(feed: dict[str, str]) -> tuple[list[RawItem], dict[str, Any]]:
        ...
        if strict and not local_items and error is None:
            error = "No dated RSS/Atom entries parsed"
...
def public_archive_only(archive: dict[str, dict[str, Any]], opml_path: Path) -> dict[str, dict[str, Any]]:
    """Drop private/extra sources and metadata from history before public reuse."""
    names = {feed["title"] for feed in parse_opml_subscriptions(opml_path)}
    fields = {"id", "site_id", "site_name", "source", "title", "url", "published_at", "first_seen_at", "last_seen_at"}
    clean = {}
    for key, record in archive.items():
        site_id = record.get("site_id")
        source = str(record.get("source") or "")
        url = urlparse(str(record.get("url") or ""))
        allowed = (
            (site_id == "opmlrss" and source in names)
            or (site_id == "official_ai" and source == "Anthropic News" and url.hostname == "www.anthropic.com")
            or (site_id == "followbuilders" and source.startswith("Follow Builders"))
        )
        if allowed and url.scheme in {"https", "http"} and url.hostname and not url.username and not url.password:
            clean[key] = {field: record[field] for field in fields if field in record}
    return clean
...
def main() -> int:
    ...
    parser.add_argument("--public-only", action="store_true", help="Use only the 10 public example feeds, Anthropic and Follow Builders; no mail, paid APIs or LLM calls")
    ...
    if args.public_only:
        if args.rss_opml:
            parser.error("--public-only uses the committed public example; private OPML is not accepted")
        args.rss_opml = str(Path(__file__).resolve().parents[1] / "feeds" / "follow.example.opml")
        args.translate_max_new = 0
        args.translate_max_new_broad = 0
    ...
    if args.public_only:
        archive = public_archive_only(archive, Path(args.rss_opml))
    paid_source_state = load_paid_source_state(paid_source_state_path)

    session = create_session()
    service_status_payload = ({"generated_at": iso(now), "enabled": False, "incidents": [], "active_count": 0}
                              if args.public_only else fetch_service_status(session, now))
    raw_items, statuses = collect_all(session, now, public_only=args.public_only)
    ...
    ) if not args.public_only else (None, {"enabled": False, "reason": "public_only"})
    ...
    x_api_items, x_api_status = (maybe_fetch_x_api_updates(session, now) if not args.public_only
                               else ([], {"enabled": False, "reason": "public_only"}))
    ...
    socialdata_items, socialdata_status = (maybe_fetch_socialdata_updates(session, now, paid_source_state) if not args.public_only
                                         else ([], {"enabled": False, "reason": "public_only"}))
    ...
    tikhub_items, tikhub_status = (maybe_fetch_tikhub_updates(session, now, paid_source_state) if not args.public_only
                                 else ([], {"enabled": False, "reason": "public_only"}))
    ...
    if args.public_only:
        waytoagi_payload = {"generated_at": iso(now), "enabled": False, "count_7d": 0, "updates_7d": []}
    else:
    ...
    if args.public_only:
        # Refuse to overwrite the published snapshot after a widespread outage.
        healthy = sum(bool(status.get("ok")) for status in rss_feed_statuses)
        healthy += sum(bool(status.get("ok")) for status in statuses if status["site_id"] != "opmlrss")
        if healthy < 6:
            raise RuntimeError(f"Only {healthy}/12 public sources succeeded; keeping the previous deployment")
        for status in rss_feed_statuses:
            if status.get("error"):
                status["error"] = "Feed fetch or parsing failed"
    ...
    for item_id, record in archive.items():
        ...
        if args.public_only:
            ts = parse_iso(record.get("published_at")) or datetime.min.replace(tzinfo=UTC)
        if ts >= keep_after:
    ...
    for record in archive.values():
        ...
        if not ts:
            continue
        if ts >= window_start and (not args.public_only or ts <= now):
            normalized = dict(record)
            normalized["title"] = maybe_fix_mojibake(str(normalized.get("title") or ""))
            normalized["source"] = maybe_fix_mojibake(normalize_source_for_display(
                str(normalized.get("site_id") or ""),
    ...
    title_cache = {} if args.public_only else load_title_zh_cache(title_cache_path)
    ...
    status_payload = {
        "profile": "public-only" if args.public_only else "full",
        "generated_at": generated_at,
        "sites": statuses,
